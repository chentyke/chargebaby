...
export function useGenericImagePreloader<T extends Product>(products: T[], enabled: boolean = true) {
  useEffect(() => {
    ...
    if (isLowPerf) {
      ...
      const preloadTimer = setTimeout(() => {
        preloadImages(products.slice(0, 6)); // 只预加载前6个
      }, 5000); // 5秒后开始预加载
...
async function preloadImages<T extends Product>(products: T[]): Promise<PreloadResult | null> {
  try {
    // 提取所有图片URL，优先选择高质量图片
